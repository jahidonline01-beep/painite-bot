...
async def check_sub(update: Update, ctx: ContextTypes.DEFAULT_TYPE, user_id: int) -> bool:
    ...
    try:
        ...
        if hasattr(member.status, 'value'):
            status_val = str(member.status.value).lower()
        return status_val in ['member', 'administrator', 'creator', 'owner']
    except Exception as e:
        # If channel ID is not accessible or bot is not admin in channel, allow user to continue
        logging.warning(f"check_sub check failed (allowing bypass): {e}")
        return True
...
async def start_command(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    welcome = (
        ...
        f"🤖 <b>Bot:</b> Painite OTP Bot\n"
        f"🛡️ <b>Engine:</b> 24/7 Active\n\n"
...
async def allocate(update: Update, ctx: ContextTypes.DEFAULT_TYPE, rid: str, change_cb: str, bypass: bool = False):
    ...
    if not rid:
        rid = await asyncio.to_thread(panel_instance.latest_rid) or "22501"
    ...
    c_flag = res.get('flag') or detect_country_from_phone(phone)[1]
    operator = res.get('operator', '')

    # Record allocated number in database
    try:
        db.add_number(phone, c_name, c_flag)
    except Exception as e:
        logging.warning(f"Failed to record number in DB: {e}")

    kb = InlineKeyboardMarkup([
        [
            InlineKeyboardButton("🔄 Change Number", callback_data=change_cb),
            InlineKeyboardButton("🔐 Get OTP", callback_data="otp_check")
        ],
        [
    ...
    msg = (
        f"📱 <b>{html_escape(c_flag)} {html_escape(c_name)} Number Ready</b>\n\n"
        f"☎️ <b>Phone Number:</b>\n"
        f"<code>{html_escape(phone)}</code>\n\n"
        f"🎯 <b>Range:</b> <code>{html_escape(rid)}XXX</code>\n"
        f"⏰ <b>Time:</b> <code>{datetime.now().strftime('%H:%M:%S')}</code>\n"
...
async def handle_paid_number(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        "Click below to message the admin and say: <i>'sir i want to buy paid number'</i>",
...
async def handle_country_menu(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if not countries:
        await update.message.reply_text(
            ...
            "The panel has no active ranges right now. Try later.",
    ...
    await update.message.reply_text(
        f"🌍 <b>Choose Country</b>\n\n📊 <b>Available:</b> {len(countries)} countries",
...
async def handle_otp_input(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if found:
        ...
        await update.message.reply_text(
            f"✅ <b>OTP Found!</b>\n\n"
            f"☎️ <b>Number:</b> <code>{html_escape(sms.get('phone', ''))}</code>\n"
            f"⚙️ <b>Service:</b> {html_escape(sms.get('service', 'Unknown'))}\n"
            f"🔑 <b>OTP:</b> {otp_display}\n\n"
            f"✉️ <b>Full Message:</b>\n{html_escape(sms.get('message', ''))}",
            parse_mode=ParseMode.HTML, reply_markup=main_menu()
        )
    else:
        await update.message.reply_text(
            f"😔 <b>No OTP Found</b>\n\n"
            f"No messages found for <code>{html_escape(input_num)}</code>.\nWait a moment and try again.",
            parse_mode=ParseMode.HTML, reply_markup=main_menu()
        )
...
async def main():
    ...
    # Global Telegram Error Handler to prevent crash / silence on network glitches
    async def global_error_handler(update: object, context: ContextTypes.DEFAULT_TYPE) -> None:
        logging.error(f"Telegram Exception in handler: {context.error}", exc_info=context.error)
